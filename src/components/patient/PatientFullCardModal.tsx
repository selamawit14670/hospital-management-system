...
import { X, IdCard, QrCode, Shield, CheckCircle2, Printer, Download, Share2, Phone, Heart, Sparkles } from 'lucide-react';
import type { PatientProfileData } from '../../types/patient';

interface PatientFullCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  profile: PatientProfileData;
}

export const PatientFullCardModal: React.FC<PatientFullCardModalProps> = ({
  isOpen,
  onClose,
  profile,
}) => {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Official encrypted medical identity & emergency card
              </p>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-6 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 text-white p-6 shadow-2xl border border-blue-700/50">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-blue-300 font-medium">
                    National Emergency Medical Registry
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4 relative z-10">
              <img
                ...
                className="w-20 h-20 rounded-2xl object-cover border-2 border-white/50 shadow-md shrink-0"
              />
              ...
              <div className="min-w-0 space-y-1">
                ...
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-blue-200">{profile.gender}</span>
                </div>

                <div className="flex items-center gap-2 text-[11px] pt-0.5">
                  <span className="text-blue-100">DOB: May 14, 1988 ({profile.age} yrs)</span>
                  <span>•</span>
            ...
            {/* Bottom Row: QR Code & Verified Hospital Network */}
            <div className="mt-6 pt-4 border-t border-white/20 flex items-end justify-between relative z-10 gap-4">
              <div className="space-y-1 text-xs text-blue-200 min-w-0">
                ...
                <p className="text-[11px] text-blue-300">
                  Primary Physician: {profile.primaryDoctor.name}
                </p>
                <p className="text-[10px] text-blue-400 font-mono">
                  Card Valid: {profile.cardIssueDate} - {profile.cardExpiryDate}
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
            <div>
              ...
              <p className="text-blue-600 font-mono">{profile.emergencyContact.phone}</p>
            </div>

            <div>
              <p className="text-[11px] text-slate-500 font-medium">Insurance Policy</p>
              <p className="font-bold text-slate-900">{profile.insurance.provider.split('(')[0]}</p>
              <p className="font-mono text-slate-600">{profile.insurance.policyNumber}</p>
            </div>

            <div>
              <p className="text-[11px] text-slate-500 font-medium">Registered Address</p>
              <p className="text-slate-800 font-medium">{profile.address}, {profile.city}</p>
            </div>

            <div>
              <p className="text-[11px] text-slate-500 font-medium">Card Status</p>
